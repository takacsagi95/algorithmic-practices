...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    List<Person> sorted = people.stream()
        .sorted(Comparator.comparing(Person::getAge)) // .reversed() --> to get the reversed order
        .toList();
    ...
    Map<Gender, List<Person>> groupByGender =  people.stream()
        .collect(Collectors.groupingBy(Person::getGender)); // Collectors.groupingBy() --> MAP WITH GENDERS
    ...
    Optional<String> oldestFemalesName = people.stream()
        .filter(person -> person.getGender().equals(Gender.FEMALE))
        .max(Comparator.comparing(Person::getAge))
        .map(Person::getName);
    oldestFemalesName.ifPresent(System.out::println);
  }

  private static List<Person> getPeople() {
    return List.of(
        new Person("James Bond", 20, Gender.MALE), // Antonio
